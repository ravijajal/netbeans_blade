 ...
 */
public class BladeIndenter extends AbstractIndenter<BladeTopTokenId> {
    private static final Logger LOGGER = Logger.getLogger(BladeIndenter.class.getName());
    private Stack<BladeStackItem> stack = null;
    ...
    public BladeIndenter(Context context) {
        super(BladeTopTokenId.language(), context);
    }

    @Override
    protected int getFormatStableStart(JoinedTokenSequence<BladeTopTokenId> ts, int startOffset, int endOffset, OffsetRanges rangesToIgnore) throws BadLocationException {
        return 0;
    }

    @Override
    protected List<IndentCommand> getLineIndent(IndenterContextData<BladeTopTokenId> context, List<IndentCommand> preliminaryNextLineIndent) throws BadLocationException {
        Stack<BladeStackItem> blockStack = getStack();
        List<IndentCommand> iis = new ArrayList<>();
        getIndentFromState(iis, true, context.getLineStartOffset());

        JoinedTokenSequence<BladeTopTokenId> ts = context.getJoinedTokenSequences();
        ...
        while (!context.isBlankLine() && ts.moveNext()
            ...
            Token<BladeTopTokenId> token = ts.token();
            if (token == null) {
                lastToken = null;
                continue;
                ...
                if (embeddingLevel == 1 && afterDelimiter) {
                    if (token.id() == BladeTopTokenId.T_BLADE_BLOCK && context.isIndentThisLine()) {
                        String markupToken = getMarkupTokenName(token);
                        isBlockMacro = BladeSyntax.isBlockMacro(markupToken) && !isShortedBlockMacro(token);
                        if (isBlockMacro) {
                            lastMacro = markupToken;
                            isElseMacro = BladeSyntax.isElseMacro(markupToken);
                        }
                    } else {
                        isBlockMacro = false;
                        isElseMacro = false;
            ...
            if (token.id() == BladeTopTokenId.T_BLADE_BLOCK_START) {
                if (lastToken != null && lastToken.id() == BladeTopTokenId.T_BLADE_BLOCK_END) {
                     nonControlMacro = true;
                }
                afterDelimiter = true;
                embeddingLevel++;
                BladeStackItem state = new BladeStackItem(StackItemState.IN_RULE);
                blockStack.push(state);
            } else if (token.id() == BladeTopTokenId.T_BLADE_BLOCK_END) {
                afterDelimiter = false;
                if (isInState(blockStack, StackItemState.IN_RULE)) {
                    // check that IN_RULE is the last state
                    BladeStackItem item = blockStack.pop();
                    embeddingLevel--;
                    if (embeddingLevel == 0) {
                        ...
                        if (isBlockMacro) {
                            if (!blockStack.isEmpty()
                                    ...
                                    && BladeSyntax.isRelatedMacro(lastMacro, blockStack.peek().getMacro())) {
                                if (isElseMacro) {
                                    String macro = blockStack.pop().getMacro();
                                    blockStack.push(new BladeStackItem(StackItemState.IN_BODY, macro));
                                } else {
                                    blockStack.pop();
                                }
                                if (!nonControlMacro) {
                                ...
                                blockStack.push(new BladeStackItem(StackItemState.IN_BODY, lastMacro));
                            }
                        }
                    }
                }
    ...
    private static boolean isShortedBlockMacro(Token<BladeTopTokenId> token) {
    ...
    private String getMarkupTokenName(Token<BladeTopTokenId> token) {
        String result = "";
        TokenSequence<? extends TokenId> sequence = null;
        if (token.id() == BladeTopTokenId.T_BLADE_BLOCK) {
            TokenHierarchy<CharSequence> th = TokenHierarchy.create(token.text(), BladeBlockTokenId.language());
            sequence = th.tokenSequence(BladeBlockTokenId.language());
        } else if (token.id() == BladeTopTokenId.T_BLADE_VAR) {
            TokenHierarchy<CharSequence> th = TokenHierarchy.create(token.text(), BladeVariableTokenId.language());
            sequence = th.tokenSequence(BladeVariableTokenId.language());
        }
        if (sequence != null) {
            while (sequence.moveNext()) {
                if (sequence.token().id() != BladeBlockTokenId.T_BLADE_WHITESPACE
                        && sequence.token().id() != BladeVariableTokenId.T_BLADE_WHITESPACE
    ...
    private boolean isWhitespaceControlModifier(Token<? extends TokenId> token) {
        TokenId id = token.id();
        if (id == BladeBlockTokenId.T_BLADE_OPERATOR || id == BladeVariableTokenId.T_BLADE_OPERATOR) {
            return BladeLexerUtils.textEquals(token.text(), '-');
        }
        return false;
    }

    private boolean isCommentToken(Token<BladeTopTokenId> token) {
        return token.id() == BladeTopTokenId.T_BLADE_COMMENT;
    }
    ...
    private void getIndentFromState(List<IndentCommand> iis, boolean updateState, int lineStartOffset) {
        ...
        for (int i = lastUnprocessedItem; i < blockStack.size(); i++) {
            BladeStackItem item = blockStack.get(i);
    ...
    private Stack<BladeStackItem> getStack() {
        return stack;
    }

    @Override
    protected boolean isWhiteSpaceToken(Token<BladeTopTokenId> token) {
        return false;
    }
    ...
    private boolean isInState(Stack<BladeStackItem> stack, StackItemState state) {
        for (BladeStackItem item : stack) {
            if (item.state == state) {
                return true;
            }
        }
    ...
    private static final class BladeStackItem {
        private StackItemState state;
        private Boolean processed = false;
        private String macro;
        private int indent;

        private BladeStackItem(StackItemState state, String macro) {
        ...
        private BladeStackItem(StackItemState state) {
            this.state = state;
            this.indent = -1;
        }
        ...
        public String toString() {
            return "BladeStackItem[state=" + state + ",indent=" + indent + ",processed=" + processed + ",command=" + macro + "]"; //NOI18N
        }
