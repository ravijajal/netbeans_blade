...
public class BladeParser extends Parser {
    ...
    public void parse(Snapshot snapshot, Task task, SourceModificationEvent sme) throws ParseException {
        ...
        for (LanguagePath path : tokenHierarchy.languagePaths()) {
            if (path.mimePath().endsWith(BladeBlockTokenId.language().mimeType())) {
                bladePath = path;
                break;
            }
        }
            ...
            for (TokenSequence<?> sequence : tokenSequenceList) {
                ...
                while (sequence.moveNext()) {
                    ...
                    while (sequence.moveNext()) {
                        ...
                        if (token.id() == BladeBlockTokenId.T_BLADE_NAME) {
                        ...
                        if (PARSE_ELEMENTS.contains(block.function.toString())) {
                            ...
                            if (CharSequenceUtilities.equals(block.function, "block")) { //NOI18N
                                ...
                                do {
                                    ...
                                    if (token.id() == BladeBlockTokenId.T_BLADE_NAME || token.id() == BladeBlockTokenId.T_BLADE_STRING) {
                                        names++;
                                    }
                                ...
                                do {
                                    ...
                                    if (token.id() == BladeBlockTokenId.T_BLADE_OPERATOR) {
                                        standalone = true;
                                        break;
                                    }
    ...
    private static boolean isWhitespaceControlOperator(Token<BladeBlockTokenId> token) {
        return token.id() == BladeBlockTokenId.T_BLADE_OPERATOR && TokenUtilities.equals("-", token.text()); // NOI18N
    }
