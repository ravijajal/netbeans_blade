...
public class BladeTypedTextInterceptor implements TypedTextInterceptor {
    ...
    public void insert(MutableContext context) throws BadLocationException {
        ...
        if (id instanceof BladeBlockTokenId || id instanceof BladeVariableTokenId) {
            // complete quote or bracket
            if (isOpeningBracket(ch) || isQuote(ch)) {
                if (selection != null && selection.length() > 0) {
                    surroundSelectionWithChars(selection, ch, context);
                } else {
                    if (id != BladeBlockTokenId.T_BLADE_STRING && id != BladeVariableTokenId.T_BLADE_STRING) {
                        if (isQuote(ch)) {
                            if (BladeLexerUtils.textStartWith(token.text(), ch)) {
                                skipQuote = true;
                            } else {
    ...
    private boolean skipQuotes(TokenSequence<? extends TokenId> ts, char quote, boolean backward) {
        ...
        while (backward ? ts.movePrevious() : ts.moveNext()) {
            ...
            if (tokenId == BladeVariableTokenId.T_BLADE_STRING
                    || tokenId == BladeBlockTokenId.T_BLADE_STRING) {
                break;
            }
            if (tokenId == BladeVariableTokenId.T_BLADE_WHITESPACE
                    || tokenId == BladeBlockTokenId.T_BLADE_WHITESPACE
                    || tokenId == BladeVariableTokenId.T_BLADE_OTHER
                    || tokenId == BladeBlockTokenId.T_BLADE_OTHER) {
    ...
    private void afterInsertUnderWriteLock(Context context) throws BadLocationException {
        ...
        switch (ch) {
            ...
            case '%':
                ...
                TokenSequence<? extends BladeTopTokenId> ts = BladeLexerUtils.getBladeTokenSequence(doc, tokenEndPos);
                if (ts == null) {
                    return;
                }
                ts.move(tokenEndPos);
