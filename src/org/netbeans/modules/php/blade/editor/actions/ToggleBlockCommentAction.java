...
public class ToggleBlockCommentAction extends BaseAction {
    ...
    public static enum ToggleCommentType {
        ...
        LANGUAGE_SENSITIVE {
            ...
            void comment(BaseDocument baseDocument, Positions positions, AtomicBoolean processedHere) throws BadLocationException {
                ...
                if (token != null && token.id() == BladeTopTokenId.T_HTML) {
                    ...
                    TokenInsertWrapper startTokenWraper = findBackward(ts, Arrays.asList(BladeTopTokenId.T_BLADE_BLOCK_START, BladeTopTokenId.T_BLADE_VAR_START));
                    TokenInsertWrapper endTokenWrapper = findForward(ts, Arrays.asList(BladeTopTokenId.T_BLADE_BLOCK_END, BladeTopTokenId.T_BLADE_VAR_END));
        ...
        private static TokenInsertWrapper findBackward(TokenSequence<? extends BladeTopTokenId> ts, List<BladeTopTokenId> tokenIds) {
            assert ts != null;
        ...
        private static TokenInsertWrapper findForward(TokenSequence<? extends BladeTopTokenId> ts, List<BladeTopTokenId> tokenIds) {
            assert ts != null;
            assert tokenIds != null;
            TokenInsertWrapper result = TokenInsertWrapper.NONE;
            ts.moveNext();
    ...
    private static final class TokenInsertWrapperImpl implements TokenInsertWrapper {
        private final Token<? extends BladeTopTokenId> token;
        private final int offset;

        private TokenInsertWrapperImpl(Token<? extends BladeTopTokenId> token, int offset) {
            this.token = token;
            this.offset = offset;
        }
        ...
        @Override
        public void insertBefore(BaseDocument baseDocument) throws BadLocationException {
            baseDocument.insertString(offset, BladeTopLexer.OPEN_COMMENT, null);
        }

        @Override
        public void insertAfter(BaseDocument baseDocument) throws BadLocationException {
            baseDocument.insertString(offset + token.text().length(), BladeTopLexer.CLOSE_COMMENT, null);
        }
